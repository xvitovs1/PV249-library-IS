...
class BorrowTest < ActiveSupport::TestCase
  def setup
    ...
    @publication.save
    @borrow = Borrow.new(borrow_date: Date.today,
                         expected_return_date: Date.tomorrow,
                         publication: @publication, reader: readers(:one))
  end

  test 'cannot borrow publication that is already borrowed' do
    @borrow.save
    Borrow.new(borrow_date: Date.today, expected_return_date: Date.tomorrow,
               publication: @publication, reader: readers(:two))
